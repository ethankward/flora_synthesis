...
from flora.models.records.inat_record.choices import observation_types as inat_observation_types
...
class TaxonTests(TestCase):
    ...
    def test_exclude_inat_casual(self):
        ...
        i1 = factory.InatRecordFactory(observation_type=inat_observation_types.InatObservationTypeChoices.RESEARCH,
                                       mapped_taxon=taxon, active=True)
        self.assertTrue(i1.active)
        self.assertTrue(taxon in i1.checklist_taxon.all_mapped_taxa.all())

        i2 = factory.InatRecordFactory(observation_type=inat_observation_types.InatObservationTypeChoices.CASUAL,
                                       mapped_taxon=taxon, active=True)
