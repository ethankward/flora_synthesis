...
def run():
    ...
    for record_model in record_models:
        for record in record_model.objects.filter(
            mapped_taxon__isnull=False,
            checklist__primary_checklist=True,
            observation_type=observation_types.SEINETObservationTypeChoices.COLLECTION,
        ).select_related("mapped_taxon", "mapped_taxon__parent_species"):
    ...
    models.Taxon.objects.filter(pk__in=taxa_with_collections).update(
