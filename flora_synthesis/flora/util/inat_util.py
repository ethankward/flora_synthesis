...
def get_canonical_rank(rank: str) -> typing.Optional[taxon_ranks.TaxonRankChoices]:
    return {
        "species": taxon_ranks.TaxonRankChoices.SPECIES,
        "hybrid": taxon_ranks.TaxonRankChoices.HYBRID,
        "variety": taxon_ranks.TaxonRankChoices.VARIETY,
        "subspecies": taxon_ranks.TaxonRankChoices.SUBSPECIES,
    }.get(rank, None)
...
class InatApi:
    ...
    def read_api_data(
            self, url: str, parameters: dict, paginate: bool = True
        ...
        if not paginate:
            ...
            for page in itertools.count(1):
                data = json.loads(
                    self.session.get(url, params={**parameters, **{"page": page}}).text
                )
                results = data["results"]
                if len(results) == 0:
                    break
    ...
    def get_urls_with_ids(
            self, base_url: str, ids: typing.Optional[typing.List[int]] = None
    ):
        if ids is None:
            yield base_url
        else:
    ...
    def read_observation_data(
            self,
            parameters: dict,
            observation_ids: typing.Optional[typing.List[int]] = None,
    ):
...
class InatRecordsReader(checklist_util.RecordReader):
    def read_records(
            self,
            records: typing.Optional[typing.List[models.InatRecord]] = None,
            limit: int = 10,
    ):
        if records is None:
...
class InatChecklistReader(checklist_util.ChecklistReader):
    ...
    def get_family(self, ancestry: str):
        ...
        for taxon_id in taxon_ids:
            try:
                existing = models.ChecklistTaxonFamily.objects.get(
                    checklist=self.checklist, external_id=taxon_id
                )
                return existing
        ...
        for taxon_data_item in self.inat_api.read_taxon_data(taxon_ids):
            if taxon_data_item["rank"] == "family":
                result = models.ChecklistTaxonFamily(
                    checklist=self.checklist,
                    external_id=taxon_data_item["id"],
    ...
    def generate_data(
        ...
        for observation_data_item in self.inat_api.read_observation_data(
                self.parameters, self.observation_ids
        ):
