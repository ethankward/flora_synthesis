...
def date_range_list(
    start_date: timezone.datetime.date, end_date: timezone.datetime.date
) -> typing.Generator[timezone.datetime.date, None, None]:
...
def combine_date_ranges(dates) -> typing.Generator[dict, None, None]:
    ...
    for year in years:
        if all(
            [
                test_date in dates_set
                for test_date in date_range_list(
                    timezone.datetime(year=year, month=1, day=1).date(),
        ...
        if date.year not in full_years:
            if all(
                ...
                if (date.year, date.month) not in full_months:
                    ...
                    created_d2 = "{}-{}-{}".format(
                        date.year, date.month, last_month_day.day
                    )
                    end_date = timezone.datetime(
                        year=date.year, month=date.month, day=last_month_day.day
                    )
                    yield {"created_d1": created_d1, "created_d2": created_d2}, end_date
    ...
    for date in dates:
        if date.year not in full_years:
            if (date.year, date.month) not in full_months:
                ...
                yield {
                    "created_on": "{}-{}-{}".format(date.year, date.month, date.day)
                }, end_date
