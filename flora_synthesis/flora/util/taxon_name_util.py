...
class TaxonName:
    ...
    def parse_subspecies(self, parts, ssp_index=2):
        ...
        self.genus = genus
        self.parent_species_name = "{} {}".format(genus, specific_epithet)
        self.canonical_name = "{} {} subsp. {}".format(
            genus, specific_epithet, subspecies
        )
    ...
    def parse_subspecies_variety(self, parts):
        ...
        self.genus = genus
        self.parent_species_name = "{} {}".format(genus, specific_epithet)
        self.canonical_name = "{} {} subsp. {} var. {}".format(
            genus, specific_epithet, subspecies, variety
        )
    ...
    def parse_hybrid_5(self, parts):
        ...
        genus = parts[0].title()
        self.genus = genus
        self.canonical_name = "{} {} × {} {}".format(
            genus, parts[1], parts[3], parts[4]
        )
    ...
    def get_db_item(self):
        ...
        if self.family is None:
            self.family = input(
                "Enter family for taxon {}: ".format(self.original_name)
            )

        if self.parent_species_name is not None:
            parent_species = TaxonName(
                self.parent_species_name, family=self.family
            ).get_db_item()
        else:
            parent_species = None
